...
AGENT_SPAWNS_SIMPLE = jnp.array(
    [
        [(j, i), (GRID_SIZE - 1 - j, GRID_SIZE - 1 - i)]
        for (i, j) in AGENT_SPAWNS_SIMPLE
    ],
    dtype=jnp.int8,
).reshape(-1, 2, 2)

class InTheGrid_2p_simple(InTheGrid_2p):
    def __init__(
        self,
        num_inner_steps=152,
        num_outer_steps=1,
        ...
        def _get_obs(state: State) -> jnp.ndarray:
            ...
            _grid2 = grid2.at[:, :, 0].set(grid2[:, :, 1])
            _grid2 = _grid2.at[:, :, 1].set(grid2[:, :, 0])
            obs2 = jnp.concatenate([_grid2, angle2], axis=-1)
            ...
            return {
                'agent_0': jnp.concatenate([obs1.ravel(), inv1.ravel()], dtype=jnp.uint8),
                'agent_1': jnp.concatenate([obs2.ravel(), inv2.ravel()], dtype=jnp.uint8), 
            }
        ...
        def _soft_reset_state(key: jnp.ndarray, state: State) -> State:
            ...
            agent_pos = jax.random.choice(
                key, AGENT_SPAWNS_SIMPLE, shape=(), replace=False
            )
        ...
        def _reset_state(
            ...
            agent_pos = jax.random.choice(
                subkey, AGENT_SPAWNS_SIMPLE, shape=(), replace=False
            )
            player_dir = jax.random.randint(
                subkey, shape=(2,), minval=0, maxval=3, dtype=jnp.int8
            )
