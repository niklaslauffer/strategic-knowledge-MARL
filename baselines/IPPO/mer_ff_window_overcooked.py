...
from jaxmarl.environments.multi_agent_env import OverridePlayer, OverridePlayer2, DelayedObsWrapper
...
def get_rollout(train_state, config):
    ...
    coparams = pickle.load(open(config['COPARAMS_FILE'], "rb"))
    first_coparams = jax.tree_util.tree_map(lambda x : x[0], coparams)
    ...
    while not done:
        ...
        # STEP ENV
        obs, state, reward, done, info = env.step_copolicy(key_s, state, actions, first_coparams)
        done = done["__all__"]
...
#     num_samples = 50
#     filename = f'{config["ENV_NAME"]}_{config["LAYOUT_NAME"]}_obsDelay{config["OBS_DELAY"]}_save'
...
@hydra.main(version_base=None, config_path="config", config_name="mer_ff_window_overcooked")
def main(config):
    ...
    filename = f'{config["ENV_NAME"]}_{layout_name}_mer_obsDelay{config["OBS_DELAY"]}'
    ...
    fig.suptitle(f'{filename}_{config["NUM_PARTICLES"]}x{config["NUM_COPOLICIES"]}')

    lines = jnp.swapaxes(mean_returns, 0, 1)
    for l in lines:
        axs[0].scatter(jnp.arange(len(l)), l)
    axs[1].plot(jnp.nanmax(lines, axis=0))
            
    axs[0].set(xlabel='Update Step', ylabel='Return')
    axs[1].set(xlabel='Update Step', ylabel='Max Return')

    plt.savefig(f'{filename}_{config["NUM_PARTICLES"]}x{config["NUM_COPOLICIES"]}.png')

    print(out["metrics"]["returned_episode_returns"].shape)
    all_returns = out["metrics"]["returned_episode_returns"].reshape((-1, config["NUM_STEPS"], config["NUM_PARTICLES"], config['NUM_COPOLICIES'], config["NUM_ENVS"]))
    last_returns = jnp.mean(all_returns, axis=-1)[-1]
    ...
    for i in range(config['NUM_PARTICLES']):
        state_seq = [jax.tree_map(lambda x : x[i], s.state) for s in v_state_obs_seq]
        viz.animate(state_seq, agent_view_size=5, filename=f'{filename}_{config["NUM_PARTICLES"]}x{config["NUM_COPOLICIES"]}_particle{i}.gif')
